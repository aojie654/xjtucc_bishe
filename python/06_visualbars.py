...
for list_file_name_0_t in list_file_name_0:
    ...
    try:
        ...
        # 打开停用词文件,读取内容
        open_stop_content_0 = open(file_source_name_0, "r")
        read_stop_content_0 = open_stop_content_0.read()
        open_stop_content_0.close()
        ...
        for tmp_of_list_fre_line in list_fre_line_0[1:]:

            # 将每行以"\t"分割,得到列表
            tmp_of_fre_line_split = tmp_of_list_fre_line.split("\t")
            # 取出词(下标为1),出现次数(下标为2)
            tmp_word_0 = tmp_of_fre_line_split[1]
            tmp_fre_0 = int(tmp_of_fre_line_split[2].replace(" ", ""))
            if (len(tmp_word_0) != 1) and (tmp_word_0 in read_stop_content_0):
                # 将词追加与词列表之后, 将出现次数(下标为2)追加于词频列表之后
                list_words_0.append(tmp_word_0)
                list_word_fre_0.append(tmp_fre_0)
            if len(list_words_0) >= 20:
                break
