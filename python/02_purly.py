...
for list_file_name_0_t in list_file_name_0:
    ...
    if "_pure.txt" in list_file_name_0_t:
        ...
        try:
            # 读取文件内容
            open_file_source_0 = open(list_file_name_0_t, "r", encoding="utf-8")
            ...
            read_content_0 = re.sub(r".*(更多小说|更多电子书|章节内容开始|内容简介).*", "", read_content_0)
            
            # 删除作者和摘录
            read_content_0 = re.sub(r".*(贾平凹.+|路遥.+)?.{1,10}(全文完|已完结).*(选自)?.{1,10}", "", read_content_0)
            ...
            if '/' in list_file_name_0_t:
                works_name_0 = list_file_name_0_t[list_file_name_0_t.rfind('/')+1:]
            else:
                works_name_0 = list_file_name_0_t
            read_content_0 = re.sub(r"^.{0,3}"+works_name_0+".*(贾平凹|路遥)?.*", "", read_content_0)
